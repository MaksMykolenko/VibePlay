...
export const AdminReports: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'dismissed': return <span className="badge badge-secondary">Dismissed</span>;
      case 'reviewing': return <span className="badge badge-warning">Reviewing</span>;
      default: return <span className="badge badge-danger">Open</span>;
    }
  };
  ...
  return (
    <div style={containerStyle} className="animate-fade">
      ...
      <div>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '4px' }}>Review complaints lodged by players regarding games or comments.</p>
      </div>
      ...
      <div style={listAreaStyle}>
        {reports.length === 0 ? (
          <div style={emptyContainerStyle}>
            ...
            <h3>No Reports</h3>
            <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>VibePlay platform is running smoothly.</span>
          </div>
        ) : (
          reports.map(r => (
            <div 
              ...
              style={{
                ...reportCardStyle,
                borderLeft: r.status === 'open' ? '4px solid var(--danger)' : '4px solid transparent'
              }}
              className="bg-glass"
            >
              ...
              <div style={metaGridStyle}>
                ...
                <div>
                  ...
                  <span style={valStyle}>{r.targetType.toUpperCase()} ({r.targetName})</span>
                </div>
              </div>
              ...
              {r.status === 'open' && (
                <div style={actionsRowStyle}>
                  <button onClick={() => handleResolve(r.id)} className="btn btn-success btn-sm" style={{ gap: '4px' }}>
                    <CheckCircle size={12} />
                    Resolve Report
                  </button>
                  <button onClick={() => handleDismiss(r.id)} className="btn btn-secondary btn-sm" style={{ gap: '4px' }}>
                    <XCircle size={12} />
                    Dismiss Report
                  </button>
                </div>
