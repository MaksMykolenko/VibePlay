...
import { Edit2, Eye, EyeOff, Trash2, ArrowUpCircle, Plus, LayoutGrid, CheckCircle } from 'lucide-react';
...
export const MyGames: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'hidden': return <span className="badge badge-secondary">Hidden</span>;
      default: return <span className="badge badge-primary">Draft</span>;
    }
  };
  ...
  const handleDelete = (gameId: string, title: string) => {
    if (window.confirm(`Are you absolutely sure you want to delete "${title}"? This action is permanent.`)) {
      deleteGame(gameId);
    ...
    <div style={containerStyle} className="animate-fade">
      ...
      <div style={headerStyle}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '4px' }}>Upload, submit, and manage versions of your browser builds.</p>
        </div>
      ...
      {myGames.length === 0 ? (
        <div style={emptyContainerStyle}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '0.25rem', marginBottom: '1.5rem', maxWidth: '300px' }}>
            Get started by uploading your first Phaser, Three.js or WebGL zip project.
          </p>
          <Link to="/creator/publish" className="btn btn-primary">Publish Your First Game</Link>
        </div>
      ) : (
              ...
              {myGames.map(game => (
                <tr key={game.id} style={tableBodyRowStyle}>
                  ...
                  <td style={tdStyle}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{game.category} • v{game.version}</div>
                  </td>
                  ...
                  <td style={tdStyle}>
                    ...
                    {game.status === 'rejected' && (
                      <div style={{ fontSize: '0.7rem', color: 'var(--danger)', marginTop: '4px', maxWidth: '180px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} title={game.rejectReason}>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right' }}>
                    <div style={actionsContainerStyle}>
                      ...
                      {/* Preview */}
                      {(game.status === 'published' || game.status === 'pending' || game.status === 'rejected') && (
                        <Link 
                          to={`/game/${game.slug}`} 
                      ...
                      {(game.status === 'published' || game.status === 'hidden') && (
                        ...
                        >
                          {game.status === 'published' ? <EyeOff size={14} /> : <CheckCircle size={14} style={{ color: 'var(--success)' }} />}
                        </button>
                      )}
