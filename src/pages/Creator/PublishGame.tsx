...
import { 
  FileCode, UploadCloud, Monitor, Smartphone, Tablet, CheckCircle, 
...
export const PublishGame: React.FC = () => {
  ...
  // --- Step 3: Media ---
  const [coverUrl, setCoverUrl] = useState('https://images.unsplash.com/photo-1542751371-adc38448a05e?w=600');
  const [screenshotUrl, setScreenshotUrl] = useState('https://images.unsplash.com/photo-1542751371-adc38448a05e?w=800');
  ...
  const handleZipFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const interval = setInterval(() => {
      ...
      if (prog === 20) {
        setUploadLog(prev => [...prev, 'Establishing secure sandboxed tunnel...', 'Uploading zip archive...']);
      } else if (prog === 60) {
        setUploadLog(prev => [...prev, 'Zip archive received. Decompressing files...', 'Scanning archive for security threats...']);
      } else if (prog === 80) {
        setUploadLog(prev => [...prev, 'Index.html found at root level.', 'Scripts and stylesheet references checked.', 'No external server requests detected. Safe for Sandbox.']);
      } else if (prog === 100) {
        setUploadLog(prev => [...prev, 'Upload complete! Diagnostic checks passed successfully.']);
        setIsUploaded(true);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const newGame = createGame({
        ...
        aiTools,
        version: '1.0.0',
        changelog: [{ version: '1.0.0', date: new Date().toISOString().split('T')[0], notes: 'Initial public launch build submitted.' }],
    ...
    return (
      <div style={successWrapperStyle} className="bg-glass animate-slide-up">
        ...
        <p style={successDescStyle}>
          ...
          Platforms diagnostic scanners completed sandboxed scanning, and found <strong>0 warnings</strong>.
        </p>
        <p style={{ ...successDescStyle, color: 'var(--text-secondary)' }}>
          Moderators typically review new builds within 24 hours. We'll send you an alert notification when the game status is updated.
        </p>
          ...
          <div key={num} style={stepIndicatorContainerStyle}>
            <div 
              style={{
                ...
                backgroundColor: step === num ? 'var(--secondary)' : step > num ? 'var(--primary)' : 'var(--bg-hover)',
      ...
      <div style={formCardStyle} className="bg-glass">
        ...
        {step === 1 && (
          <div className="animate-fade">
            ...
            <p style={stepDescStyle}>Provide a catchy title and clear descriptions to attract players.</p>
        ...
        {step === 2 && (
          <div className="animate-fade">
            ...
            <p style={stepDescStyle}>
              Upload a `.zip` archive containing your index file. 
              Games are scanned and run inside an isolated browser sandbox. Server-side code is not supported.
            </p>
            ...
            <div style={uploadContainerStyle}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                Maximum file size limit: 50MB. Make sure `index.html` is located at the archive root folder.
              </span>
            </div>
            ...
            {zipFileName && (
              <div style={fileDetailsBoxStyle} className="bg-glass">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{zipFileSize}</span>
                </div>

                {/* Progress bar */}
                <div style={{ height: '6px', width: '100%', backgroundColor: 'var(--bg-hover)', borderRadius: '3px', overflow: 'hidden', marginBottom: '1rem' }}>
                  <div style={{ height: '100%', width: `${uploadProgress}%`, backgroundColor: uploadProgress === 100 ? 'var(--success)' : 'var(--secondary)', transition: 'width 0.3s ease' }}></div>
                </div>

                {/* Scanners logs console */}
                <div style={consoleLogStyle}>
                  {uploadLog.map((log, idx) => (
                    <div key={idx} style={{ color: log.includes('passed') || log.includes('complete') ? 'var(--success)' : 'var(--text-secondary)' }}>
                      &gt; {log}
                    </div>
                  ))}
                </div>
          ...
          <div className="animate-fade">
            ...
            <div className="form-group">
              ...
              <input
                ...
                className="form-input"
              />
              <span style={helperTextStyle}>Enter an Unsplash or absolute image link (aspect ratio 16:10).</span>
            </div>
            ...
            <div style={previewBoxStyle} className="bg-glass">
              <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '8px' }}>Cover Preview:</div>
              <img src={coverUrl} alt="Cover Preview" style={previewImgStyle} onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=600'; }} />
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="animate-fade">
            ...
            <p style={stepDescStyle}>Specify which systems and input controls are required for gameplay.</p>

            <div style={flexGroupStyle}>
              <div style={{ flex: 1 }}>
                <label className="form-label" style={{ fontWeight: 600 }}>Devices Compatibility</label>
                <div style={checkboxStackStyle}>
                  <label className="checkbox-group">
                    ...
                    />
                    <span style={{ fontSize: '0.9rem' }}><Monitor size={14} style={{ marginRight: '6px' }} /> Desktop support</span>
                  </label>
                  ...
                  <label className="checkbox-group">
                    <input
                      ...
                      className="checkbox-input"
                    />
                    <span style={{ fontSize: '0.9rem' }}><Smartphone size={14} style={{ marginRight: '6px' }} /> Mobile support</span>
                  </label>
                  ...
                  <label className="checkbox-group">
                    <input
                      ...
                      className="checkbox-input"
                    />
                    <span style={{ fontSize: '0.9rem' }}><Tablet size={14} style={{ marginRight: '6px' }} /> Tablet support</span>
                  </label>
                </div>
              </div>
            ...
            <div className="form-group">
              <label className="checkbox-group">
                <input
                  ...
                  className="checkbox-input"
                />
                <span style={{ fontSize: '0.9rem' }}>This game features Multiplayer capabilities (Simulated WebSockets)</span>
              </label>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="animate-fade">
            ...
            <p style={stepDescStyle}>Honest disclosure helps players discover and understand your development tools.</p>

            <div className="form-group">
              <label className="form-label">Was AI used to create this game? *</label>
              <div style={radioRowStyle}>
                <label style={{ ...radioItemStyle, borderColor: aiDisclosure === 'no' ? 'var(--primary)' : 'var(--border-color)', backgroundColor: aiDisclosure === 'no' ? 'var(--primary-soft)' : 'transparent' }}>
                  <input
                    type="radio"
                    name="aiDisclosure"
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '2px' }}>Built entirely by human programming.</div>
                  </div>
                </label>

                <label style={{ ...radioItemStyle, borderColor: aiDisclosure === 'assisted' ? 'var(--primary)' : 'var(--border-color)', backgroundColor: aiDisclosure === 'assisted' ? 'var(--primary-soft)' : 'transparent' }}>
                  <input
                    type="radio"
                    name="aiDisclosure"
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '2px' }}>Generative AI helped draft scripts, debug, or design templates.</div>
                  </div>
                </label>

                <label style={{ ...radioItemStyle, borderColor: aiDisclosure === 'generated' ? 'var(--primary)' : 'var(--border-color)', backgroundColor: aiDisclosure === 'generated' ? 'var(--primary-soft)' : 'transparent' }}>
                  <input
                    type="radio"
                    name="aiDisclosure"
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '2px' }}>Core coding logic and textures were generated from prompts.</div>
                  </div>
                </label>
              </div>
            </div>
            ...
            {aiDisclosure !== 'no' && (
              <div className="form-group" style={{ marginTop: '1.5rem' }}>
                ...
                <div style={aiToolsGridStyle}>
                  {['Claude', 'Gemini', 'GPT-4 / ChatGPT', 'Cursor', 'Midjourney', 'v0', 'Suno'].map(tool => (
        ...
        {step === 6 && (
          <div className="animate-fade">
            ...
            <p style={stepDescStyle}>Review all details before committing build to the moderation queue.</p>
            ...
            <div style={reviewGridStyle} className="bg-glass">
              ...
              <div style={reviewDetailsStyle}>
                ...
                <span className="badge badge-success" style={{ alignSelf: 'flex-start' }}>{category}</span>
                <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginTop: '8px' }}>{shortDesc}</p>
                
                <hr style={{ border: 'none', borderTop: '1px solid var(--border-color)', margin: '12px 0' }} />
                
                <div style={reviewMetaRowStyle}>
                  <div><strong>Build Name:</strong> {zipFileName} ({zipFileSize})</div>
                  <div><strong>AI Status:</strong> {aiDisclosure === 'no' ? 'Human-made' : `AI ${aiDisclosure} (${aiTools.join(', ')})`}</div>
                  <div><strong>Compatibilities:</strong> {deviceDesktop ? 'Desktop ' : ''}{deviceMobile ? 'Mobile ' : ''}{deviceTablet ? 'Tablet ' : ''}</div>
                  <div><strong>Multiplayer:</strong> {multiplayer ? 'Yes' : 'No'}</div>
                </div>
              </div>
            </div>
            ...
            <div style={noticeBoxStyle}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                Submitting a build locks metadata edits until the moderation review finishes. The file size, scan checksums, and author details will be published in public logs.
              </span>
            </div>
          </div>
        )}
        ...
        <div style={wizardFooterStyle}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
            <button onClick={() => navigate('/creator')} className="btn btn-secondary btn-sm" style={{ border: 'none' }}>
              Cancel
            </button>
            {step < 6 ? (
              ...
              <button onClick={handleSubmit} disabled={loading} className="btn btn-primary" style={{ gap: '6px' }}>
                <Sparkles size={16} />
